...
namespace Stations.DataProcessor
{
	public static class Deserializer
	{
	    ...
	    public static string ImportClasses(StationsDbContext context, string jsonString)
		{
		    ...
		    var  deserialezedClasses= JsonConvert.DeserializeObject<SeatingClassDto[]>(jsonString);

		    var  validClasses= new List<SeatingClass>();

		    foreach (var seatingClassDto in deserialezedClasses)
		    {
		        ...
		        var seatingClassAlreadyExists = validClasses.Any(c => c.Name == seatingClassDto.Name || c.Abbreviation == seatingClassDto.Abbreviation);
	    ...
	    {
	        var deserializedTrips =
	            JsonConvert.DeserializeObject<TripDto[]>(jsonString, new JsonSerializerSettings()
                {
	                NullValueHandling = NullValueHandling.Ignore
	            });
		    ...
		    foreach (var tripDto in deserializedTrips)
		    {
		        ...
		        var arrivelTime = DateTime.ParseExact(tripDto.ArrivalTime, "dd/MM/yyyy HH:mm", CultureInfo.InvariantCulture);
                
		        TimeSpan timeDiffrerence;

		        if (tripDto.TimeDifference != null)
		        {
		            timeDiffrerence = TimeSpan.ParseExact(tripDto.TimeDifference, @"hh\:mm", CultureInfo.InvariantCulture);
                }

		        if (departureTime > arrivelTime)
		        {
		            sb.AppendLine(FailureMessage);
		            continue;
                }
		        ...
		        var trip = new Trip()
		        {
		            ...
		            Status = status,
		            TimeDifference = timeDiffrerence
		        };
        ...
        private static bool IsValid(object obj)
        {
            ...
            var validationResult = new LinkedList<ValidationResult>();
