    ...
    public class ConnectionHandler
    {
        ...
        private async Task<IHttpResponse> HandleRequest(IHttpRequest httpRequest)
        {
            ...
            else
            {
                ...
                var response = this.requestHandler.Handle(httpRequest);
        ...
        {
            var responseByte = Encoding.UTF8.GetBytes(httpResponse.ToString()).ToList();

            if (httpResponse is FileResponse)
            {
                responseByte.AddRange(((FileResponse)httpResponse).FileData);
            }
